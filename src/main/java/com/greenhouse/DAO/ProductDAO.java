...
public interface ProductDAO extends JpaRepository<Product, Integer> {
    ...
    @Query(value = "select * from Products where Id in ( select Product_Id from Set_Category sc  where sc.Category_Id = ?1) AND Status = 1", nativeQuery = true)
    List<Product> findByCategory(String category);

    @Query(value = "select * from Products where Id in ( select Product_Id from Set_Category sc  where sc.Category_Id = ?1) AND Price <= ?2 AND Status = 1", nativeQuery = true)
    List<Product> findByCategoryAndPrice(String category, String price);

    @Query(value = "select * from Products where Quantity > 0 AND Price <= ?1 AND Status = 1", nativeQuery = true)
